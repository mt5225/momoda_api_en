var cmds = [
	{
		...
		"parameters":[	
			...
			{ "parameter":"enable", "des":"设定是否显示界面", "requred":true }
		],
		"examples":[
			{
				"name":"",
	...
	
	{
		...
		"cmdDes":"创建界面模板。",
		"parameters":[
			{ "parameter":"template", "des":"要设定模板的内容，具体内容看下方例子。", "requred":true }
		],
		"examples":[
			{
				"name":"",
...

	{
		...
		"parameters":[
			{ "parameter":"originName", "des":"要复制的模板名字" , "requred":true },
			{ "parameter":"newName", "des":"复制后新的模板名字", "requred":true }
		],
		"examples":[
			{
				"name":"",
	...
	
	{
		...
		"cmdDes":"删除指定模板",
		"parameters":[
			{ "parameter":"name", "des":"指定要删除的模板名字" , "requred":true }
		],
		"examples":[
			{
				"name":"",
	...
	
	{
		...
		"parameters":[
			{ "parameter":"name", "des":"指定要修改的模板名字" , "requred":true },
			{ "parameter":"path", "des":"指定要修改哪个控件的哪个属性,中间使用'/'分割" , "requred":true },
			{ "parameter":"config", "des":"要修改的值" , "requred":true }
		],
		"examples":[
			{
				"name":"",
	...
	},	
	{
		...
		"parameters":[
			{ "parameter":"templateName", "des":"创建时使用的模板名字" , "requred":true },
			{ "parameter":"placemarkId", "des":"可以指定创建的界面的名字。<br>	" +
					"注：如果通过ObjectCommand输入物体，标示界面是创建到物体身上，此时可以不输入placemarkId。那是系统会内置使用 '物体uid_模板名字'作为placemarkId" , "requred":false },
			{ "parameter":"type", "des":"指定界面是创建到3D里还是浮于屏幕上<br>	" +
					"可设置选项：<br>	" +
					"	Screen：标示这个界面是在屏幕坐标下创建的，大小不会随着摄影机远近而变化<br>	" +
					"	Plane：标示这个界面是在3D坐标下创建的，大小会随着摄影机远近而变化" , "requred":false, "default":"Screen" },
//			{ "parameter":"worldSize", "des":"指定要删除的模板名字" , "requred":true },
//			{ "parameter":"track", "des":"指定要删除的模板名字" , "requred":true },
//			{ "parameter":"axis", "des":"指定要删除的模板名字" , "requred":true },
			{ "parameter":"scale", "des":"当type设置成Plane，界面在3D中显示时，默认的大小是使用模板里定义的像素点大小变成使用'米制'，这里通过scale来控制界面的大小" , "requred":false, "default":[0.2, 0.2, 0.2] },			
			{ "parameter":"presetPos", "des":"可以指定界面放置的位置。在type设置为Plane时，输入三维坐标， 在type设置为Screen时，输入二维坐标。<br>	" +
					"注：如果没有指定位置，界面又是创建在物体上时，默认显示位置是物体的头顶" , "requred":false },
			{ "parameter":"posFromBuffer", "des":"同presetPos，通过buffer输入位置" , "requred":false },
			{ "parameter":"layoutOffset", "des":"当创建在物体上，默认显示位置是物体的头顶，这个参数可以再设定一个偏移量。在type设置为Plane时，输入三维坐标， 在type设置为Screen时，输入二维坐标。" , "requred":false },
			{ "parameter":"turnWhenNegative", "des":"输入位置的坐标是二维坐标时，如果坐标是负值：<br>	" +
					...
					"当turnWhenNegative设置成false，标示坐标是在屏幕外侧。比如[-0.1, -100],表示距屏幕左边-10%,上面-100像素点的位置,就是在屏幕外了。<br>	" , "requred":false, "default":false },
			{ "parameter":"countPerFrame", "des":"使用命令可以同时给多个物体创建头顶的界面，这个参数控制每帧可同时创建的数量，以优化显示。当设为0时，标示一次创建所有的界面，如果数量很多可能会卡死一段时间。" , "requred":false, "default":0   },
			{ "parameter":"additiveDataObjects", "des":"当界面创建在物体上，物体作为宿主可以给界面提供数据。但当界面需要多个物体数据源，或者界面是个二维屏幕界面，没有创建在物体上，有需要物体提供数据时，可以通过这个参数控制物体数据的来源。<br>	" +
					"使用键值对可输入多个数据源，键用于模板里引用属性路径，值可以输入字符或者buffer，字符代表物体的id或者uid，所有如果输入buffer，不能使用字符代表Buffer的简写方式。" , "requred":false },			
			{ "parameter":"updateTime", "des":"界面上如果连接了物体显示实时数据，这个参数控制界面刷新数据的时间间隔，单位是秒" , "requred":false, "default":1 }
	...
	},		
	{
		...
		"parameterInherit":"ObjectCommand",
		"parameters":[
			{ "parameter":"placemarkId", "des":"创建时设置了placemarkId，这里需要输入placemarkId" , "requred":false },
			{ "parameter":"templateName", "des":"创建时没有设置placemarkId，系统会内置使用 '物体uid_模板名字'作为placemarkId，所有使用这种方法创建时，需要传入templateName" , "requred":false }
	...
	},
	{
		...
		"parameters":[
			{ "parameter":"toBuffer", "des":"设置物体宿主存放的buffer" , "requred":true }
		],
		"examples":[
			{
				"name":"",
	...
	},	
	{
		...
		"parameters":[
			{ "parameter":"placemarkId", "des":"创建时设置了placemarkId，这里需要输入placemarkId" , "requred":false },
			{ "parameter":"templateName", "des":"创建时没有设置placemarkId，系统会内置使用 '物体uid_模板名字'作为placemarkId，所有使用这种方法创建时，需要传入templateName" , "requred":false },
			{ "parameter":"widgetName", "des":"要控制的控件的名字" , "requred":true },
			{ "parameter":"property", "des":"要设置的控件的属性名" , "requred":true },
...

	{
		...
		"parameters":[
			{ "parameter":"config", "des":"配置图层有哪些选项" , "requred":true },
	...
	
	{
		...
		"parameters":[
			{ "parameter":"items", "des":"通过数组可同时控制多个功能项" , "requred":true },
			{ "parameter":"state", "des":"输入true或false，指定是否是勾选状态" , "requred":true }
		],
		"examples":[
			{
				"name":"",
	...
	
	{
		...
		"cmdDes":"设置是否显示功能面板",
		"parameters":[			
			{ "parameter":"enable", "des":"指定要删除的模板名字" , "requred":true }
		],
		"examples":[
			{
				"name":"",
...

	{
		...
		"parameterInherit":"ObjectCommand",
		"parameters":[
			{ "parameter":"config", "des":"可以配置多个功能按钮" , "requred":true },
			{ "parameter":"presetPos", "des":"可以指定界面放置的位置，输入二维坐标。<br>	" +
					"注：如果没有指定位置，界面又是创建在物体上时，默认显示位置是物体的头顶" , "requred":false },
			{ "parameter":"posFromBuffer", "des":"同presetPos，通过buffer输入位置" , "requred":false },
			{ "parameter":"turnWhenNegative", "des":"输入位置的坐标是二维坐标时，如果坐标是负值：<br>	" +
	...
	
	{
		...
		"parameters":[
			{ "parameter":"items", "des":"通过数组可同时控制多个按钮项" , "requred":true },
			{ "parameter":"state", "des":"输入true或false，指定是否是点击状态" , "requred":true },
			{ "parameter":"exeCommands", "des":"设置状态为true时,是否运行命令" , "requred":false, "default":false },
		],
		"examples":[
			{
				"name":"",
	...
	},
	{
		...
		"parameters":[
			{ "parameter":"valueBuffer", "des":"需要先把属性值放到buffer里，这里引入buffer，提供过给面板显示数据" , "requred":true },
			{ "parameter":"names", "des":"从buffer里取值，按照names的内容和次序" , "requred":false }
		],
		"examples":[
			{
				"name":"",
